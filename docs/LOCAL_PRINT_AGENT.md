...
- A Windows Scheduled Task named `BizBil Print Agent` starts it at user login.
...
Run PowerShell as Administrator, then run these commands from the BizBil repo on the Windows billing computer:

```powershell
cd D:\suri-projects\bizbil
corepack enable
corepack pnpm install --frozen-lockfile
corepack pnpm --filter @bizbil/print-agent build
...
corepack pnpm install --frozen-lockfile
corepack pnpm --filter @bizbil/print-agent build
Stop-ScheduledTask -TaskName "BizBil Print Agent" -ErrorAction SilentlyContinue
powershell -ExecutionPolicy Bypass -File apps/print-agent/scripts/install-startup-task.ps1
```
...
```powershell
Get-ScheduledTask -TaskName "BizBil Print Agent" | Select-Object TaskName, State
```
...
2. Set `Connection type` to `BizBil Local Agent`.
...
```powershell
Start-ScheduledTask -TaskName "BizBil Print Agent"
Invoke-RestMethod http://127.0.0.1:9211/health
```
...
```powershell
Get-ScheduledTaskInfo -TaskName "BizBil Print Agent" | Select-Object LastRunTime, LastTaskResult
```
