    ...
    <div className="space-y-4">
      ...
      {data.customer.creditLimit != null && (
        <div className="rounded-md border border-amber-200 bg-amber-50 px-4 py-2 text-sm text-amber-800">
          Credit limit: INR {Number(data.customer.creditLimit).toLocaleString("en-IN")}
          {data.outstanding > Number(data.customer.creditLimit) && (
                ...
                {data.entries.map((entry) => (
                  <tr key={entry.id}>
                    ...
                    <td className={`px-4 py-2 text-right text-xs font-medium ${entry.type === "payment" ? "text-emerald-700" : "text-slate-900"}`}>
                      {entry.type === "payment" ? "−" : "+"}INR {Number(entry.amount).toLocaleString("en-IN", { minimumFractionDigits: 2 })}
                    </td>
                    <td className={`px-4 py-2 text-right text-xs font-semibold ${entry.balance > 0 ? "text-red-600" : "text-slate-500"}`}>
                      INR {Number(entry.balance).toLocaleString("en-IN", { minimumFractionDigits: 2 })}
                    </td>
                  </tr>
                ))}
              </tbody>
