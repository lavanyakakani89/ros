...
export function encryptPhonePeSecret(secret: string): string {
...
export function decryptPhonePeSecret(ciphertext: string | null | undefined): string | null {
  if (!ciphertext) {
    return null;
  }
...
function getEncryptionKey(): Buffer {
  const source = process.env.PHONEPE_SECRET_ENCRYPTION_KEY ?? process.env.JWT_SECRET ?? "bizbil-local-phonepe-secret-key";
