import type { Customer, Quotation, QuotationItem, Tenant } from "@prisma/client";
...
export async function generateQuotationPdf(input: {
  ...
  const html = template({
    ...
    customer: input.quotation.customer ?? null,
    quotationDate: input.quotation.createdAt.toLocaleDateString("en-IN", { timeZone: "Asia/Kolkata" }),
    validUntil: input.quotation.validUntil?.toLocaleDateString("en-IN", { timeZone: "Asia/Kolkata" }) ?? "-",
    ...
    subtotal: money(input.quotation.subtotal),
    totalDiscount: money(input.quotation.totalDiscount),
    totalCgst: money(input.quotation.totalCgst),
    totalSgst: money(input.quotation.totalSgst),
    grandTotal: money(input.quotation.grandTotal),
  ...
  try {
    ...
    const filename = `quotations/${input.tenant.id}/${input.quotation.id}.pdf`;
  ...
  <body>
    <section class="header">
      ...
      <div>
        ...
        <div class="muted">No: {{quotation.quotationNumber}}</div>
        <div class="muted">Date: {{quotationDate}}</div>
