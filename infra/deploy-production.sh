...
if [[ -z "$COMPOSE_PROJECT_NAME_VALUE" ]] && command -v docker >/dev/null 2>&1; then
  ...
  if docker volume inspect "${LEGACY_COMPOSE_PROJECT_NAME}_postgres_data" >/dev/null 2>&1 &&
    ! docker volume inspect "bizbil-prod_postgres_data" >/dev/null 2>&1; then
    echo "==> Reusing existing production compose project ${LEGACY_COMPOSE_PROJECT_NAME} to preserve persisted volumes"
